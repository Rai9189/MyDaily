...
import {
  ...
  Loader2, CalendarX, CheckCircle2, Clock, CalendarClock, ChevronRight, ChevronDown, BarChart2,
} from 'lucide-react';
...
export function Dashboard() {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case 'urgent':   return <Badge className="text-xs bg-orange-500 hover:bg-orange-600 text-white">Urgent</Badge>;
      case 'upcoming': return <Badge className="text-xs bg-amber-400 hover:bg-amber-500 text-white">Upcoming</Badge>;
      default:         return <Badge variant="secondary" className="text-xs">On Track</Badge>;
    }
  };
    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex-1 overflow-y-auto no-scrollbar">
        <div className="space-y-4 pb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-card border border-border rounded-lg text-xs font-medium shadow-sm hover:bg-muted transition-colors"
              >
                ...
                <ChevronDown size={13} className={`text-muted-foreground transition-transform ${accountDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {accountDropdownOpen && (
                <>
          ...
          {/* ── Balance + Income + Expense — equal 3 cols ── */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <Card className="bg-primary text-primary-foreground border-0 shadow-lg rounded-xl">
              <CardContent className="pt-4 pb-4 px-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="opacity-10 hidden md:block"><Wallet size={44} /></div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white dark:bg-card border-2 border-green-200 dark:border-green-900/50 shadow-sm rounded-xl">
              <CardContent className="pt-4 pb-4 px-4">
                ...
                <p className="text-2xl font-bold text-foreground">{fmt(income)}</p>
                <p className="text-xs text-muted-foreground mt-1">
                  {filteredTx.filter(t => t.type === 'income').length} transactions
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white dark:bg-card border-2 border-red-200 dark:border-red-900/50 shadow-sm rounded-xl">
              <CardContent className="pt-4 pb-4 px-4">
                ...
                <p className="text-2xl font-bold text-foreground">{fmt(expense)}</p>
                <p className="text-xs text-muted-foreground mt-1">
                  {filteredTx.filter(t => t.type === 'expense').length} transactions
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className="bg-white dark:bg-card border-2 border-blue-200 dark:border-blue-900/50 shadow-sm rounded-xl">
              <CardHeader className="pb-0 pt-4 px-4">
                ...
                <div className="flex items-center justify-between mb-3">
                  <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-2"><BarChart2 size={15} className="text-primary" />Transaction Chart</CardTitle>
                  <button
                    type="button"
                ...
                {/* Row 2: toggle full width */}
                <div className="flex bg-muted rounded-lg p-1 gap-1">
                  {([
                    { key: 'income',  label: 'Income'           },
                    { key: 'expense', label: 'Expense'          },
                    { key: 'both',    label: 'Income vs Expense' },
                  ...
                  <>
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        ...
                        {pieMode !== 'both' && (
                          <Legend
                            formatter={v => (
                              <span style={{ fontSize: '11px', color: 'var(--foreground)' }}>{v}</span>
                            )}
                          />
                        )}
                      </PieChart>
                    ...
                    <div className="space-y-1.5 mt-2">
                      {pieData.slice(0, 5).map((item, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-muted/40">
              ...
              <CardContent className="px-4 pb-4">
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                  {[
                    ...
                    <div key={item.label} className="flex flex-col items-center bg-slate-50 dark:bg-muted/40 rounded-lg py-2.5 px-1">
                      {item.icon}
                      <p className={`text-lg font-bold mt-1 leading-none ${item.color}`}>{item.count}</p>
