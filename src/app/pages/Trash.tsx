...
const DEFAULT_CONFIRM: ConfirmState = {
  ...
  variant: 'danger', icon: null, onConfirm: () => {},
};

const TABLE_LABELS: Record<string, string> = {
  transactions: 'Transaction', tasks: 'Task', notes: 'Note', categories: 'Category', accounts: 'Account',
};
const TABLE_ICONS: Record<string, React.ReactNode> = {
  transactions: <Receipt size={13} />, tasks: <CheckSquare size={13} />, notes: <StickyNote size={13} />,
  categories: <Tag size={13} />, accounts: <Wallet size={13} />,
};
  ...
  return (
    <div className="space-y-4 mt-1">
      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${TABLE_COLORS[item.table]}`}>
        {TABLE_ICONS[item.table]}{TABLE_LABELS[item.table]}
      </span>
      <div className="space-y-3 divide-y divide-border">
        ...
        <div className="space-y-3 pt-3">
          ...
          <DetailRow label="Time remaining" value={
            <span className={`font-medium flex items-center gap-1 ${days <= 3 ? 'text-red-600 dark:text-red-400' : days <= 7 ? 'text-amber-600 dark:text-amber-400' : 'text-foreground'}`}>
...
export function Trash() {
  ...
  const filtered = (activeTab === 'all' ? trashItems : trashItems.filter(i => i.table === activeTab))
    .slice().sort((a, b) =>
      sortBy === 'expiry'
        ? daysUntilExpiry(a.deleted_at) - daysUntilExpiry(b.deleted_at)
        : new Date(b.deleted_at).getTime() - new Date(a.deleted_at).getTime()
    );

  const countOf = (t: TableFilter) => t === 'all' ? trashItems.length : trashItems.filter(i => i.table === t).length;

  const handleRestore = async (item: TrashItem) => {
    setProcessingId(item.id + '_restore');
    const { success, error } = await restoreItem(item);
    if (success) toast.success(`${TABLE_LABELS[item.table]} restored!`);
    else toast.error(error || 'Failed to restore');
  ...
  const handleRestoreAll = () => {
    setConfirmState({
      ...
      description: `Restore all ${filtered.length} item${filtered.length !== 1 ? 's' : ''} from trash?`,
  ...
  const handleClearAll = () => {
    setConfirmState({
      open: true,
      title: 'Empty Trash',
      description: `All ${trashItems.length} item${trashItems.length !== 1 ? 's' : ''} will be permanently deleted. This action cannot be undone.`,
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
        <div className="space-y-4 pb-6">
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            {trashItems.length > 0 && (
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" className="gap-2" onClick={handleRestoreAll} disabled={restoringAll || clearingAll}>
                  {restoringAll ? <><Loader2 size={14} className="animate-spin" /> Restoring...</> : <><RotateCcw size={14} /> Restore All</>}
                </Button>
                <Button variant="destructive" size="sm" className="gap-2" onClick={handleClearAll} disabled={clearingAll || restoringAll}>
                  {clearingAll ? <><Loader2 size={14} className="animate-spin" /> Clearing...</> : <><Trash2 size={14} /> Empty Trash</>}
                </Button>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
              {TAB_FILTERS.map(t => {
                const count = countOf(t); const active = activeTab === t;
                return (
                  <button key={t} type="button" onClick={() => setActiveTab(t)}
                    ...
                    <span className={`ml-0.5 px-1.5 py-0.5 rounded-full text-xs font-bold ${active ? 'bg-white/20 text-white' : 'bg-muted text-muted-foreground'}`}>{count}</span>
                  </button>
                );
              })}
            </div>
                    ...
                    return (
                      <div key={item.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
                        ...
                        <div className="flex items-center gap-0.5 flex-shrink-0">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => setDetailItem(item)}><Eye size={14} /></Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary hover:bg-primary/10"
                            onClick={() => handleRestore(item)} disabled={isProcessing || restoringAll}>
                          ...
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:bg-red-500 hover:text-white"
                            onClick={() => handleHardDelete(item)} disabled={isProcessing || restoringAll}>
            ...
            <>
              ...
              <div className="flex gap-2 pt-3 border-t border-border mt-1">
                <Button variant="outline" className="flex-1 gap-2"
                  onClick={() => { setDetailItem(null); handleRestore(detailItem); }} disabled={!!processingId}>
                  <RotateCcw size={14} /> Restore
                </Button>
                <Button variant="destructive" className="flex-1 gap-2"
                  onClick={() => { setDetailItem(null); handleHardDelete(detailItem); }} disabled={!!processingId}>
