  ...
  const NoteCardView = ({ note, index = 0 }: { note: any; index?: number }) => {
    ...
    return (
      ...
      <Card onClick={() => navigate(`/notes/${note.id}`)}
        className={`hover:shadow-lg transition-shadow bg-white dark:bg-card cursor-pointer h-full flex flex-col ${note.pinned ? 'border-2 border-amber-400 dark:border-amber-500' : 'border-2 border-blue-200 dark:border-blue-900/50'}`}>
              ...
              <div>
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {note.pinned && (
                    ...
                    >
                      {pinningId === note.id ? <Loader2 size={13} className="animate-spin" /> : <Pin size={13} />}
                    </button>
                  )}
                </div>
                <h3 className="text-sm font-semibold text-foreground line-clamp-1 mt-2">{note.title}</h3>
                <p className="text-sm text-muted-foreground line-clamp-3 mt-1">{displaySnippet}</p>
