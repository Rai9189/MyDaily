...
function SettingRow({
  ...
  icon: React.ReactNode; label: React.ReactNode; description?: string;
  onClick?: () => void; disabled?: boolean; badge?: React.ReactNode; children?: React.ReactNode;
}) {
  return (
    <div className="space-y-2">
      <button type="button" onClick={onClick} disabled={disabled}
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-muted/30 hover:bg-muted transition-colors text-left ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
...
function SettingSection({ icon, title, borderColor = 'border-blue-200 dark:border-blue-900/50', children }: {
  icon: React.ReactNode; title: string; borderColor?: string; children: React.ReactNode;
}) {
  return (
...
export function Settings() {
  ...
  const handleVerifyOldPin = async (e: React.FormEvent) => {
    ...
    if (success) {
      setShowPinVerify(false); setOldPin(''); setPinVerifyAttempts(0);
        ...
        <div className="space-y-4 pb-6">
          ...
          <SettingSection icon={<Lock size={15} />} title="Security">
            ...
            >
              {resetSent && (
                <button type="button"
                  ...
                  disabled={sendingReset} className="text-xs text-primary hover:underline px-1">
                  Resend email
                </button>
              )}
            </SettingRow>
            ...
            <SettingRow
              ...
              onClick={() => {
                if (pinLocked) return;
                setShowPinVerify(v => !v); setOldPin(''); setPinVerifyError(null);
              }}
              disabled={pinLocked}
            >
              {showPinVerify && !pinLocked && (
                <form onSubmit={handleVerifyOldPin} className="space-y-3 p-3 bg-muted/40 rounded-lg border border-border mt-1">
                  <p className="text-xs text-muted-foreground">Enter your current PIN to continue</p>
                  {pinVerifyError && (
                    <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 text-xs text-red-600 dark:text-red-400">
                      {pinVerifyError}
                    </div>
                  )}
                  ...
                  <div className="flex gap-2">
                    <Button type="submit" className="flex-1 gap-2" disabled={verifyingPin || !oldPin}>
                      {verifyingPin ? <><Loader2 className="w-4 h-4 animate-spin" /> Verifying...</> : 'Verify & Continue'}
                    </Button>
          ...
          <SettingSection icon={<Tag size={15} />} title="Data" borderColor="border-amber-300 dark:border-amber-900/50">
            ...
            <SettingRow icon={<Trash2 size={15} />} label="View Trash"
          ...
          <SettingSection icon={<Sun size={15} />} title="Theme" borderColor="border-purple-300 dark:border-purple-900/50">
            ...
            <p className="text-xs text-muted-foreground px-1">Choose light, dark, or follow your system preference.</p>
          </SettingSection>

          <div className="flex items-center justify-between px-1 pt-1 pb-2">
            <p className="text-xs text-muted-foreground/60">MyDaily</p>
            <p className="text-xs text-muted-foreground/60">Version 1.11.0</p>
          </div>
        </div>
      </div>
    </div>
