...
import {
  ...
  AlertCircle, Trash2, Mail, ShieldAlert, Lock, ChevronRight,
} from 'lucide-react';
...
export function Profile() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    if (success) { setDisplayName(formData.name); toast.success('Profile updated successfully!'); }
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!['image/jpeg', 'image/jpg', 'image/png', 'image/webp'].includes(file.type)) {
      toast.error('Only JPEG, PNG, and WebP images are supported'); return;
    }
    ...
    try {
      ...
      if (success) toast.success('Profile photo updated!');
            ...
            <CardContent className="pt-5 pb-5 px-5">
              <div className="flex items-center gap-5">
                <div className="relative flex-shrink-0">
                  {user.avatar ? (
                    <img src={user.avatar} alt={user.name} className="w-20 h-20 rounded-full object-cover ring-3 ring-white/50 shadow-lg" />
                  ) : (
                    <div className="w-20 h-20 rounded-full bg-white/25 flex items-center justify-center text-3xl font-bold ring-3 ring-white/50 shadow-lg">{initials}</div>
                  )}
            ...
            <Card className="bg-white dark:bg-card border-2 border-blue-200 dark:border-blue-900/50 shadow-sm rounded-xl">
              <CardContent className="pt-4 pb-4 px-4 space-y-3">
                <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                  <UserIcon size={15} className="text-primary" /> Personal Information
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="email" className="flex items-center gap-1.5">
                      <Mail size={12} className="text-muted-foreground" /> Email
                    </Label>
                    <Input id="email" type="email" value={formData.email} disabled className="opacity-50 cursor-not-allowed" />
                ...
                <Button type="submit"
                  ...
                  {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : <><Save size={14} /> Save Changes</>}
                </Button>
              </CardContent>
            </Card>
          </form>
              ...
              {!showDeleteConfirm ? (
                ...
                <div className="space-y-3 p-3.5 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/50 rounded-lg">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input id="deleteConfirm" value={deleteConfirmText}
                      onChange={(e) => setDeleteConfirmText(e.target.value)}
                      placeholder="Type DELETE here" disabled={deletingAccount}
                      className="border-red-200 dark:border-red-800/50 focus:border-destructive text-sm" />
                  </div>
                  <div className="flex gap-2">
                    <Button type="button" variant="destructive" className="flex-1 gap-2 text-sm"
                      onClick={handleDeleteAccount} disabled={deleteConfirmText !== 'DELETE' || deletingAccount}>
                      {deletingAccount ? <><Loader2 size={13} className="animate-spin" /> Deleting...</> : <><Trash2 size={13} /> Confirm Delete</>}
                    </Button>
                    <Button type="button" variant="outline" className="text-sm"
                      onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(''); }} disabled={deletingAccount}>
                      Cancel
                    </Button>
                  </div>
                </div>
